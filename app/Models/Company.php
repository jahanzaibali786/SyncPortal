...
class Company extends BaseModel
{
    ...
    public static function fetchNumber($type = null)
    {
        if($type != null){
        ...
        // dd(request()->all());
        try {
            $voucher = JournalEntry::where('company_id', company()->id)
                ->where('voucher_type', request()->type)
                ->orderBy('id', 'desc')
                ->first();
            $number = $voucher ? ($voucher->number + 1) : 1;
