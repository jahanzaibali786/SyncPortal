...
// dd($invoice);
    ...
    public function destroy($id)
    {
        try {
            ...
            // If any Credit Notes are attached, unlink them safely
            try {
                if (CreditNotes::where('invoice_id', $id)->exists()) {
                    CreditNotes::where('invoice_id', $id)->update(['invoice_id' => null]);
                }
            } catch (\Throwable $e) {
                \Log::warning("CreditNotes unlink failed for Invoice ID {$id}: " . $e->getMessage());
            }

            // Delete journal vouchers safely
            try {
                if (!empty($invoice->journal_entry_id)) {

                    $jv = JournalEntry::find($invoice->journal_entry_id);

                    if ($jv) {
                        // Delete lines safely, depending on column name
                        try {
                            if (\Schema::hasColumn('journal_entry_lines', 'journal_entry_id')) {
                                JournalEntryLine::where('journal_entry_id', $jv->id)->delete();
                            } elseif (\Schema::hasColumn('journal_entry_lines', 'journal_id')) {
                                JournalEntryLine::where('journal_id', $jv->id)->delete();
                            }
                        } catch (\Throwable $lineError) {
                            \Log::warning("Journal lines cleanup skipped for Invoice ID {$id}: " . $lineError->getMessage());
                        }
                ...
                \Log::warning("JournalEntry deletion skipped for Invoice ID {$id}: " . $e->getMessage());
            }
            ...
            try {
                ...
                \Log::error("Invoice deletion failed for ID {$id}: " . $e->getMessage());
            }
    ...
    public function update(UpdateInvoice $request, $id)
    {
        ...
        //edit voucher
        Utility::updateInvoiceJV($invoice);
        if (user()) {
