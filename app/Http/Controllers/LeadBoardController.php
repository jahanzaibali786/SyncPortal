            ...
            $boardColumns = PipelineStage::withCount([
                'deals as deals_count' => function ($q) use ($startDate, $endDate, $request) {
                    ...
                    if ($this->viewLeadPermission == 'owned') {
                        ...
                        //         foreach ($this->myAgentId as $id) {
                        //             $subQuery->orWhereRaw("FIND_IN_SET(?, deals.sub_agents)", [$id]);
                        //         }
                        //         $subQuery->orWhereIn('deals.agent_id', $this->myAgentId);
                        //     })
            ...
            foreach ($boardColumns as $key => $boardColumn) {
                ...
                if ($this->viewLeadPermission == 'owned') {
                    $leads->where(function ($query) {
                        ...
                        $query->orWhere('deals.deal_watcher', $userId)
                            ->orWhere(function ($q) use ($userId) {
                                $q->whereRaw(
                                    "FIND_IN_SET(?, sub_agents)",
                                    [$userId]
                                );
                            });
                    });
