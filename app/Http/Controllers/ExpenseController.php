...
class ExpenseController extends AccountBaseController
{
    ...
    public function changeStatus(Request $request)
    {
        ...
        if ($expense->status == 'approved' && $expense->journal_id == null) {
            $pvexp = Utility::createPaymentVoucher($expense);
            $expense->journal_id = $pvexp->id;
        }
    ...
    public function store(StoreExpense $request)
    {
        ...
        try {
            ...
            if ($request->hasFile('bill')) {
                $filename = Files::uploadLocalOrS3($request->bill, JournalEntry::FILE_PATH);
            ...
            if ($expense->journal_id == null) {
                $jvexp = Utility::createExpenseVoucher($expense);
                $expense->journal_id = $jvexp->id;
            }
            // dd($expense);
            if ($expense->status == 'approved') {
                // Then create the payment voucher (CPV or BPV)
                $pvexp = Utility::createPaymentVoucher($expense);
            }
    ...
    public function update(StoreExpense $request, $id)
    {
        ...
        $expense->save();
        if ($expense->journal_id != null) {
            $jvexp = Utility::updateExpenseJVVoucher($expense, $expense->journal_id);
        } else {
            $jvexp = Utility::createExpenseVoucher($expense);
            $expense->journal_id = $jvexp->id;
            $expense->save();
        ...
        if ($expense->status == 'approved') {
            // Then create the payment voucher (CPV or BPV)
            $pvexp = Utility::updateExpensePaymentVoucher($expense);
        }
        // To add custom fields data
        if ($request->custom_fields_data) {
            $expense->updateCustomFieldData($request->custom_fields_data);
    ...
    public function destroy($id)
    {
        try {
            ...
            abort_403(!($this->deletePermission == 'all' ||
                ($this->deletePermission == 'added' && $this->expense->added_by == user()->id)));

            // Only proceed if journal exists and model class is valid
            if (!empty($this->expense->journal_id)) {

                // Check if JournalEntry model/table actually works
                $jv = JournalEntry::find($this->expense->journal_id);

                if ($jv) {
                    // Safely delete related lines (if column exists)
                    try {
                        if (\Schema::hasColumn('journal_entry_lines', 'journal_id')) {
                            JournalEntryLine::where('journal_id', $jv->id)->delete();
                        } elseif (\Schema::hasColumn('journal_entry_lines', 'journal_entry_id')) {
                            JournalEntryLine::where('journal_entry_id', $jv->id)->delete();
                        }
                    } catch (\Throwable $e) {
                        \Log::warning("Journal line deletion skipped: " . $e->getMessage());
                    }

                    $jv->delete();
                }

                // payment voucher (BPV/CPV)
                try {
                    $journalEntry = JournalEntry::where('source_type', 'App\Models\Expense')
                        ->where('source_id', $this->expense->id)
                        ->where(function ($q) {
                            $q->where('voucher_type', 'BPV')
                                ->orWhere('voucher_type', 'CPV');
                        })
                        ->first();

                    if ($journalEntry) {
                        if (\Schema::hasColumn('journal_entry_lines', 'journal_id')) {
                            JournalEntryLine::where('journal_id', $journalEntry->id)->delete();
                        } elseif (\Schema::hasColumn('journal_entry_lines', 'journal_entry_id')) {
                    ...
                    \Log::warning("Payment voucher cleanup skipped: " . $e->getMessage());
                }
            }
            ...
            // Catch anything unexpected — prevent app crash
            \Log::error('Expense deletion failed: ' . $e->getMessage());
            return Reply::error('Something went wrong during deletion, but no data was corrupted.');
        }
    }
