...
Route::get('/download-db', function () {
    ...
    // 🧱 Path to mysqldump (update if needed)
    // Example for XAMPP on Windows
    $mysqldumpPath = 'C:\\xampp\\mysql\\bin\\mysqldump.exe';

    // 🧠 Handle password
    $passwordOption = $dbPass ? "--password={$dbPass}" : '';

    // 🧩 Build dump command (Windows-compatible)
    $command = sprintf(
    ...
    // 🖥️ Run using Windows shell (cmd.exe)
    $process = Process::fromShellCommandline("cmd /C {$command}");
...
Route::group(['middleware' => ['auth', 'multi-company-select', 'email_verified'], 'prefix' => 'account'], function () {
    ...
    Route::get('transfers/{transfer}/edit', [EmployeeTransferController::class, 'edit'])->name('transfers.edit');
    Route::put('transfers/{transfer}', [EmployeeTransferController::class, 'update'])->name('transfers.update');
    Route::delete('transfers/{transfer}', [EmployeeTransferController::class, 'destroy'])->name('transfers.destroy');
    ...
    Route::get('mark_notification_read', [NotificationController::class, 'all'])->name('role-notifications.index');
