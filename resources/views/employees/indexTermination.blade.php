...
@section('filter-section')
    {{-- Reuse the same filter UI you already have. Nothing changes here. --}}
    {{-- @include('employees.partials._employees_filter_box') --}}
    {{-- resources/views/employees/partials/_employees_filter_box.blade.php --}}

@php
    // Safe defaults so partial can be dropped into other pages too
...
@php
    /** controller passes: $employees, $designations, $departments, $roles */
    $kind = 'termination';
@endphp
        ...
        <div class="d-flex justify-content-between action-bar">
            <div id="table-actions" class="d-block d-lg-flex align-items-center">
                <x-forms.link-primary
                    :link="route('terminations.create')"
            ...
            {{-- optional bulk actions, if you want them here --}}
            <x-datatable.actions>
                {{-- keep empty or add specific quick actions for exits if needed --}}
            </x-datatable.actions>
        </div>

        <div class="d-flex flex-column w-tables rounded mt-3 bg-white table-responsive">
            {!! $dataTable->table([
                'class' => 'table table-hover border-0 w-100',
                'id'    => 'employee-exits-table'
            ]) !!}
        </div>
    </div>
@endsection
...
@push('scripts')
    ...
    {{-- If your big filter box posts params, keep the exact same JS you already use to push filters into preXhr.dt --}}
    <script>
        // Example: nudge your existing filter script to refresh this table id
        const showExitsTable = () => window.LaravelDataTables['employee-exits-table'].draw(true);

        // If you’re reusing your existing filter code, ensure it calls showExitsTable() instead of showTable()
        // and binds to #employee-exits-table in preXhr.dt (or add a small preXhr block like below):

        $('#employee-exits-table').on('preXhr.dt', function (e, settings, data) {
            data['employee']   = $('#employee').val();
            data['searchText'] = $('#search-text-field').val();
        });

        // Example search box hook (if not already present)
        $('#search-text-field').on('keyup', showExitsTable);
        $('#employee').on('change', showExitsTable);
    </script>
@endpush
