...
<div class="modal-body">
    <x-table class="table-bordered" headType="thead-light">
        <x-slot name="thead">
            ...
            <th>@lang('modules.accounts.coa')</th> {{-- New header --}}
            ...
            <tr id="row-{{ $item->id }}">
                ...
                <td>
                    <div class='form-group mb-0'>
                        <select name="cat_role[]" data-row-id="{{ $item->id }}" id="cat_role-{{ $item->id }}"
                            multiple class="form-control select-picker cat_roles" data-size="8">
                            @foreach ($roles as $role)
                                ...
                                @foreach ($item->roles as $roledata)
                                    ...
                                    @endif
                                @endforeach
                                <option {{ $selected }} value="{{ $role->id }}">{{ $role->display_name }}
                                </option>
                            @endforeach
                        </select>
                    </div>
                ...
                <td>
                    <div class='form-group mb-0'>
                        <select name="chart_account_id" data-row-id="{{ $item->id }}"
                            id="chart_account_id-{{ $item->id }}" class="form-control select-picker chart_account"
                ...
                <td class="text-right">
                    @if (
                        $deleteExpenseCategoryPermission == 'all' ||
                            ($deleteExpenseCategoryPermission == 'added' && $item->added_by == user()->id))
    ...
    <x-form id="createProjectCategory">
        <div class="row border-top-grey ">
            <div class="col-sm-12">
                <x-forms.text fieldId="category_name" :fieldLabel="__('modules.projectCategory.categoryName')" fieldName="category_name" fieldRequired="true"
                    :fieldPlaceholder="__('placeholders.category')">
                </x-forms.text>
            </div>
            <div class="col-lg-6 col-md-6">
                <x-forms.label class="my-3" fieldId="chart_account_id" :fieldRequired="true" :fieldLabel="__('modules.accounts.coa')">
                </x-forms.label>
                <x-forms.input-group>
        ...
        if (userId != '') {
            setTimeout(function() {
                $.easyAjax({
                    ...
                    success: function(response) {
                        $('#expense_category_id').html('<option value="">--</option>' +
                            response.category);
                        $('#expense_category_id').selectpicker('refresh')
                    }
                });
        ...
        if (categoryId != '') {
            ...
            setTimeout(function() {
                $.easyAjax({
                    ...
                    success: function(response) {
                        $('#user_id').html('<option value="">--</option>' + response
                            .employees);
