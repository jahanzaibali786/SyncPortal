...
<div class="content-wrapper">
    <div class="d-flex justify-content-between align-items-center mb-3">
        <a href="{{ route('complaints.create') }}" class="btn btn-primary openRightModal">@lang('app.add')</a>
    </div>

    <div class="w-tables rounded mt-3 bg-white table-responsive">
    {!! $dataTable->table(['id'=>'trips-table','class'=>'table table-hover border-0 w-100']) !!}
    </div>
</div>
...
@push('scripts')
    @includeIf('sections.datatable_js')
    {!! $dataTable->scripts() !!}

<script>
(function () {
  const tableId = 'complaints-table';
  const DT = () => window.LaravelDataTables?.[tableId];

  // Send filter values before every ajax call
  $('#'+tableId).on('preXhr.dt', function (e, settings, data) {
    data['employee']   = $('#employee').val();
    data['searchText'] = $('#search-text-field').val();

    // UI-only (won't do anything unless you add backend support later)
  ...
  // Delete (delegated so it survives table redraws)
  $(document).off('click', '.delete-complaint').on('click', '.delete-complaint', function () {
