    ...
    @if (in_array('leads', user_modules()) && in_array('total_lead_conversions', $activeWidgets))
        <div class="col-xl-4 col-lg-6 col-md-6 mb-3">
            <a href="javascript:;" id="totalLeadConversions">
                <x-cards.widget :title="__('modules.deal.dealConversions')" :value="$convertedDeals .
                    '/' .
                    $totalLeadConversions->count() .
                    ' (' .
                    $convertDealPercentage .
                    '%)'" icon="check" :info="__('messages.leadConversion')" />
            </a>
        </div>
    @endif
    ...
    @if (in_array('contracts', user_modules()) && in_array('total_contracts_generated', $activeWidgets))
        <div class="col-xl-4 col-lg-6 col-md-6 mb-3">
            <a href="javascript:;" id="totalContractsGenerated">
                <x-cards.widget :title="__('modules.dashboard.totalContractsGenerated')" :value="$totalContractsGenerated" icon="file-contract" />
            </a>
        </div>
    @endif
                ...
                <div class="card-body p-0" id="leadStageData">
                    @if (array_sum($leadStatusChart['values']) > 0)
                        <a href="javascript:;" class="text-darkest-grey f-w-500 piechart-full-screen"
                            data-chart-id="task-chart3" data-chart-data="{{ json_encode($leadStatusChart) }}"><i
                                class="fas fa-expand float-right mr-3"></i></a>
                    @endif
                    <x-pie-chart id="task-chart3" :labels="$leadStatusChart['labels']" :values="$leadStatusChart['values']" :colors="$leadStatusChart['colors']"
        ...
        <div class="col-sm-12 col-lg-6 mt-3">
            <x-cards.data :title="__('modules.dashboard.leadVsSource')">
                @if (array_sum($leadSourceChart['values']) > 0)
                    <a href="javascript:;" class="text-darkest-grey f-w-500 piechart-full-screen"
                        data-chart-id="task-chart4" data-chart-data="{{ json_encode($leadSourceChart) }}"><i
                            class="fas fa-expand float-right"></i></a>
                @endif
                <x-pie-chart id="task-chart4" :labels="$leadSourceChart['labels']" :values="$leadSourceChart['values']" :colors="$leadSourceChart['colors']" height="300"
                    width="300" />
            </x-cards.data>
        </div>
    @endif
                    ...
                    @forelse ($latestClient->users as $item)
                        <tr>
                            ...
                            <td class="pr-20" align="right">
                                {{ $item->created_at->translatedFormat(company()->date_format) }}</td>
                        </tr>
                    @empty
                        <tr>
...
<script>
    ...
    $('#pipeline').on('change keyup', function() {
        ...
        var pipelineId = $('#pipeline').val();
        var url = "{{ route('dashboard.deal-stage-data', ':id') }}?startDate=" + startDate + "&endDate=" +
            endDate;
