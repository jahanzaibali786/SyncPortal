...
<div class="row">
    @if (in_array('tickets', user_modules()) && in_array('total_tickets', $activeWidgets))
        ...
        <div class="col-xl-6 col-lg-6 col-md-12 mb-3">
            <a href="javascript:;" class="totalTicketCount" data-status="open">
                <x-cards.widget :title="__('modules.dashboard.totalUnresolvedTickets')" :value="$totalUnresolvedTickets" icon="ticket-alt" :info="__('messages.unresolveTicketInfo')" />
            </a>
        </div>

        {{-- Resolved Tickets --}}
        <div class="col-xl-6 col-lg-6 col-md-12 mb-3">
            <a href="javascript:;" class="totalTicketCount" data-status="resolved">
                <x-cards.widget :title="__('modules.dashboard.totalResolvedTickets')" :value="$totalResolvedTickets" icon="ticket-alt" :info="__('messages.resolveTicketInfo')" />
            </a>
        </div>
    @endif
    ...
    @if (in_array('tickets', user_modules()) && in_array('total_unassigned_ticket', $activeWidgets))
        <div class="col-lg-6 col-md-6 mb-3">
            <a href="javascript:;" id="totalUnassignedTicket">
                <x-cards.widget :title="__('modules.dashboard.totalUnassignedTicket')" :value="$totalUnassignedTicket" :info="__('messages.unassignTicketInfo')" icon="ticket-alt" />
            </a>
        </div>
    @endif
            ...
            <x-cards.data :title="__('modules.dashboard.typeWiseTicket')">
                @if (array_sum($ticketTypeChart['values']) > 0)
                    <a href="javascript:;" class="text-darkest-grey f-w-500 piechart-full-screen"
                        data-chart-id="task-chart1" data-chart-data="{{ json_encode($ticketTypeChart) }}"><i
                            class="fas fa-expand float-right mr-3"></i></a>
                @endif
                <x-pie-chart id="task-chart1" :labels="$ticketTypeChart['labels']" :values="$ticketTypeChart['values']" :colors="$ticketTypeChart['colors']" height="300"
                    width="300" />
            </x-cards.data>
        </div>
    @endif
        ...
        <div class="col-sm-12 col-lg-6 mt-3">
            <x-cards.data :title="__('modules.dashboard.statusWiseTicket')">
                @if (array_sum($ticketStatusChart['values']) > 0)
                    <a href="javascript:;" class="text-darkest-grey f-w-500 piechart-full-screen"
                        data-chart-id="task-chart2" data-chart-data="{{ json_encode($ticketStatusChart) }}"><i
                            class="fas fa-expand float-right mr-3"></i></a>
                @endif
                <x-pie-chart id="task-chart2" :labels="$ticketStatusChart['labels']" :values="$ticketStatusChart['values']" :colors="$ticketStatusChart['colors']" height="300"
                    width="300" />
            </x-cards.data>
        </div>
    @endif
            ...
            <x-cards.data :title="__('modules.dashboard.channelWiseTicket')">
                @if (isset($ticketChannelChart['colors']) && $ticketChannelChart['labels'])
                    @if (array_sum($ticketChannelChart['values']) > 0)
                        <a href="javascript:;" class="text-darkest-grey f-w-500 piechart-full-screen"
                            data-chart-id="task-chart3" data-chart-data="{{ json_encode($ticketChannelChart) }}"><i
                                class="fas fa-expand float-right mr-3"></i></a>
                    @endif
                    <x-pie-chart id="task-chart3" :labels="$ticketChannelChart['labels']" :values="$ticketChannelChart['values']" :colors="$ticketChannelChart['colors']"
                    ...
                    @forelse ($newTickets as $item)
                        <tr>
                            ...
                            <td class="text-darkest-grey">
                                {{ $item->updated_at->translatedFormat(company()->date_format) }}</td>
