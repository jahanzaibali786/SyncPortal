...
<div class="row">
    @if (in_array('leaves', user_modules()) && in_array('total_leaves_approved', $activeWidgets))
        <div class="col-xl-3 col-lg-6 col-md-6 mb-3">
            ...
            <a href="javascript:;" id="total-leaves-approved">
                <x-cards.widget :title="__('modules.dashboard.totalLeavesApproved')" :value="$totalLeavesApproved" icon="plane-departure" :info="__('messages.leaveInfo')" />
            </a>
        </div>
    @endif
    ...
    @if (in_array('employees', user_modules()) && in_array('total_new_employee', $activeWidgets))
        {{-- Total Employees --}}
        <div class="col-xl-3 col-lg-6 col-md-6 mb-3">
            <a href="javascript:;" class="total-employees" data-status="open">
                <x-cards.widget :title="__('modules.dashboard.totalEmployees')" :value="$totalEmployee" icon="users" :info="__('messages.totalEmployeeInfo')" />
            </a>
        </div>

        {{-- Total New Employees --}}
        <div class="col-xl-3 col-lg-6 col-md-6 mb-3">
            <a href="javascript:;" class="total-new-employees" data-status="resolved">
                <x-cards.widget :title="__('modules.dashboard.totalNewEmployee')" :value="$totalNewEmployee" icon="users" :info="__('messages.newEmployeeInfo')" />
            </a>
        </div>
    @endif
    ...
    @if (in_array('employees', user_modules()) && in_array('total_employee_exits', $activeWidgets))
        <div class="col-xl-3 col-lg-6 col-md-6 mb-3">
            <a href="javascript:;" id="total-ex-employees">
                <x-cards.widget :title="__('modules.dashboard.totalEmployeeExits')" :value="$totalEmployeeExits" icon="sign-out-alt" :info="__('messages.employeeExitInfo')" />
            </a>
        </div>
    @endif
    ...
    @if (in_array('attendance', user_modules()) && in_array('total_today_attendance', $activeWidgets))
        <div class="col-xl-3 col-lg-6 col-md-6">
            <a href="{{ route('attendances.index') }}">
                <x-cards.widget :title="__('modules.dashboard.totalTodayAttendance')" :value="$counts->totalTodayAttendance . '/' . $counts->totalEmployees" icon="calendar-check">
                </x-cards.widget>
            </a>
        </div>
    @endif
    ...
    @if (in_array('attendance', user_modules()) && in_array('average_attendance', $activeWidgets))
        <div class="col-xl-3 col-lg-6 col-md-6 mb-3">
            <a href="{{ route('attendances.index') }}">
                <x-cards.widget :title="__('modules.dashboard.averageAttendance')" :value="$averageAttendance" icon="fingerprint" />
            </a>
        </div>
    @endif
...
<div class="row">
    @if (in_array('employees', user_modules()) && in_array('department_wise_employee', $activeWidgets))
        <div class="col-sm-12 col-lg-6 mt-3">
            <x-cards.data :title="__('modules.dashboard.departmentWiseEmployee') .
                ' <i class=\'fa fa-question-circle\' data-toggle=\'popover\' data-placement=\'top\' data-content=\'' .
                __('messages.dateFilterNotApplied') .
                '\' data-trigger=\'hover\'></i>'">
                @if (array_sum($departmentWiseChart['values']) > 0)
                    <a href="javascript:;" class="text-darkest-grey f-w-500 piechart-full-screen"
                        data-chart-id="task-chart1" data-chart-data="{{ json_encode($departmentWiseChart) }}"><i
                            class="fas fa-expand float-right mr-3"></i></a>
                @endif
                <x-pie-chart id="task-chart1" :labels="$departmentWiseChart['labels']" :values="$departmentWiseChart['values']" :colors="$departmentWiseChart['colors'] ?? null" height="300"
                    width="300" />
            </x-cards.data>
        </div>
    @endif
        ...
        <div class="col-sm-12 col-lg-6 mt-3">
            <x-cards.data :title="__('modules.dashboard.designationWiseEmployee') .
                ...
                @if (array_sum($designationWiseChart['values']) > 0)
                    <a href="javascript:;" class="text-darkest-grey f-w-500 piechart-full-screen"
                        data-chart-id="task-chart2" data-chart-data="{{ json_encode($designationWiseChart) }}"><i
                            class="fas fa-expand float-right mr-3"></i></a>
                @endif
                <x-pie-chart id="task-chart2" :labels="$designationWiseChart['labels']" :values="$designationWiseChart['values']" :colors="$designationWiseChart['colors'] ?? null" height="300"
                    width="300" />
            </x-cards.data>
        </div>
    @endif
        ...
        <div class="col-sm-12 col-lg-6 mt-3">
            <x-cards.data :title="__('modules.dashboard.genderWiseEmployee') .
                ...
                @if (array_sum($genderWiseChart['values']) > 0)
                    <a href="javascript:;" class="text-darkest-grey f-w-500 piechart-full-screen"
                        data-chart-id="task-chart3" data-chart-data="{{ json_encode($genderWiseChart) }}"><i
                            class="fas fa-expand float-right mr-3"></i></a>
                @endif
                <x-pie-chart id="task-chart3" :labels="$genderWiseChart['labels']" :values="$genderWiseChart['values']" :colors="$genderWiseChart['colors'] ?? null" height="300"
                    width="300" />
            </x-cards.data>
        </div>
    @endif
        ...
        <div class="col-sm-12 col-lg-6 mt-3">
            <x-cards.data :title="__('modules.dashboard.roleWiseEmployee') .
                ...
                @if (array_sum($roleWiseChart['values']) > 0)
                    <a href="javascript:;" class="text-darkest-grey f-w-500 piechart-full-screen"
                        data-chart-id="task-chart4" data-chart-data="{{ json_encode($roleWiseChart) }}"><i
                            class="fas fa-expand float-right mr-3"></i></a>
                @endif
                <x-pie-chart id="task-chart4" :labels="$roleWiseChart['labels']" :values="$roleWiseChart['values']" :colors="$roleWiseChart['colors'] ?? null" height="300"
                    width="300" />
            </x-cards.data>
        </div>
    @endif
    ...
    @if (in_array('employees', user_modules()) && in_array('headcount', $activeWidgets))
        <div class="col-sm-12 col-lg-12 mt-3">
            <x-cards.data :title="__('modules.dashboard.headcount') .
                ' <i class=\'fa fa-question-circle\' data-toggle=\'popover\' data-placement=\'top\' data-content=\'' .
                __('app.lastTweleveMonths') .
                ' \' data-trigger=\'hover\'></i>'">
    ...
    @if (in_array('employees', user_modules()) && in_array('joining_vs_attrition', $activeWidgets))
        <div class="col-sm-12 col-lg-12 mt-3">
            <x-cards.data :title="__('modules.dashboard.joiningVsAttrition') .
                ' <i class=\'fa fa-question-circle\' data-toggle=\'popover\' data-placement=\'top\' data-content=\'' .
                __('app.lastTweleveMonths') .
                ' \' data-trigger=\'hover\'></i>'">
                    ...
                    @forelse ($lateAttendanceMarks as $item)
                        <tr>
                            ...
                            <td class="pr-20 text-right">
                                <x-forms.button-secondary icon="eye" data-user-id="{{ $item->id }}"
                                    class="view-late-attendance">
                                    @lang('app.view')
...
<script>
    ...
    $('body').on('click', '#total-ex-employees', function() {
        ...
        string =
            `?EmployeeType=ex_employee&lastStartDate=${dateRange.startDate}&lastEndDate=${dateRange.endDate}`;
        url += string;
